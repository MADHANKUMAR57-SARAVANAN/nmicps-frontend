...
@Component({
  ...
  templateUrl: './rls-teacherstrain-creation.component.html',
  styleUrls: ['./rls-teacherstrain-creation.component.scss']
})
export class RLSteacherstrainCreationComponent implements OnInit {
  ...
  fileChange(element) {
   ...
   OnDeviceformSubmit(formdata) {
    ...
    for (var i = 0; i < this.uploadedFiles.length; i++) {
     ...
     this.dynamicScriptLoader.fileupload('uploadtrainedteacher', formData).subscribe(data => {
        console.log(data);
          if (data['status']) {
