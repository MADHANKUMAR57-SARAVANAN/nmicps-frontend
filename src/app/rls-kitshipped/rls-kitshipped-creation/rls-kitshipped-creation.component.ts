...
@Component({
  ...
  templateUrl: './rls-kitshipped-creation.component.html',
  styleUrls: ['./rls-kitshipped-creation.component.scss']
})
export class RLSkitshippedCreationComponent implements OnInit {
  ...
  OnDeviceformSubmit(formdata) {
    ...
    // }
     ...
     this.dynamicScriptLoader.fileupload('uploadfile', formData).subscribe(data => {
        console.log(data);
          if (data['status']) {
