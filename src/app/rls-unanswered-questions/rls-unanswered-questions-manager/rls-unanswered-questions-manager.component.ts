...
@Component({
  ...
  templateUrl: './rls-unanswered-questions-manager.component.html',
  styleUrls: ['./rls-unanswered-questions-manager.component.scss']
})
export class RLSUnansweredquestionManagerComponent implements OnInit {
...
curdistrictsval = '';
  ...
  get weightage() {
    return this.deviceAssign.get('weightage')
  } 
  get status() {
    return this.deviceAssign.get('status')
  } 
  get districtId() {
 ...
 addDevice() {
    this.router.navigate(['rls-unanswered-questions/rls-unanswered-questions-creation']);
  }
  ...
  deletefunction(id)
  {
    this.rest.getpost('device/deleteunans', {'Id':id}).subscribe(data => {
      this.toastr.successToastr(data.message); 

    });
  }

testfunction(IsDeleted)
{
  return (typeof IsDeleted != 'undefined' && IsDeleted =='1')?true:false;
}

testfunction1(IsAnswered)
{
  return (typeof IsAnswered != 'undefined' && IsAnswered =='1')?true:false;
}
